...
namespace KekUploadApp
{
    public class MainWindowViewModel 
    {
        ...
        public void OnStartDownloadButtonClicked()
        {
            ...
            client.ProgressChangedEvent += (totalDownloadSize, totalBytesRead, progressPercentage) =>
            {
                Dispatcher.UIThread.InvokeAsync((() =>
                {
                    ...
                    instance.DownloadProgressTextBlock.Text = totalDownloadSize != null ? $"{Utils.SizeToString(totalBytesRead)}/{Utils.SizeToString((long)totalDownloadSize)} downloaded" : $"{Utils.SizeToString(totalBytesRead)} downloaded";
        ...
        public async void OnUploadBrowseButtonClicked()
        {
            ...
            {
                AllowMultiple = false,
                SuggestedStartLocation = await instance.StorageProvider.TryGetFolderFromPathAsync(Directory.GetParent(instance.UploadFilePathTextBox.Text ?? Environment.CurrentDirectory)?.FullName ?? Environment.CurrentDirectory),
        ...
        public async void OnDownloadBrowseButtonClicked()
        {
            ...
            if (!(instance.AutoDetectNameCheckBox.IsChecked ?? false))
            {
                ...
                {
                    AllowMultiple = false,
                    SuggestedStartLocation = await instance.StorageProvider.TryGetFolderFromPathAsync(Directory.GetParent(instance.DownloadFilePathTextBox.Text ?? Environment.CurrentDirectory)?.FullName ?? Environment.CurrentDirectory),
            ...
            else
            {
                ...
                {
                    SuggestedStartLocation = await instance.StorageProvider.TryGetFolderFromPathAsync(instance.DownloadFilePathTextBox.Text ?? Environment.CurrentDirectory),
